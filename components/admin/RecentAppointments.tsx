
import { Badge } from "../ui/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "../ui/card";
import { Calendar } from "lucide-react";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "../ui/table";
import { Button } from "../ui/button";
import {useEffect, useState} from "react";
import axios from "axios";
import {toast} from "sonner";
import DoctorSkeleton from "@/components/admin/DoctorSkeleton";
import {Skeleton} from "@/components/ui/skeleton";

function RecentAppointments() {
  const [loadingData, setLoadingData] = useState<boolean>(false)
  const [appointments, setAppointments] = useState<any>([])

  useEffect(() => {
    const fetchData = async () => {
      try {
    setLoadingData(true)


        const appointmentsRes = await axios.get("/api/appointments/getAll");
        setAppointments(appointmentsRes.data.appointments);
        await cleanupAppointments()
        setLoadingData(false);
      } catch (error) {
        console.error(error);
        setLoadingData(false);
      }
    };

    fetchData();
  }, []);



  const handleToggleAppointmentStatus = async (appointmentId: string) => {

        try {
          const appointment = appointments.find((apt) => apt.id === appointmentId);

          const newStatus = appointment?.status === "ПОДТВЕРЖДЕНО" ? "ЗАВЕРШЕНО" : "ПОДТВЕРЖДЕНО";
          setAppointments((prev: any[]) =>
              prev.map((apt) =>
                  apt.id === appointmentId
                      ? { ...apt, status: newStatus }
                      : apt
              )
          );
          const res = await axios.put('/api/appointments/update', {
            id: appointmentId,
            status: newStatus
          })
          if (res.data.ok) {

            toast.success('Вы успешно обновили статус')
          }

        } catch (error) {
          console.log(error)
          toast.error('Не удалось обновить статус у приема')
        }

  };


  const getStatusBadge = (status: string) => {
    switch (status) {
      case "ПОДТВЕРЖДЕНО":
        return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">ПОДТВЕРЖДЕНО</Badge>;
      case "ЗАВЕРШЕНО":
        return <Badge className="bg-green-100 text-green-800 hover:bg-green-100">ЗАВЕРШЕНО</Badge>;
      default:
        return <Badge variant="secondary">{status}</Badge>;
    }
  };
  const cleanupAppointments = async () => {
    try {
       await axios.delete('/api/appointments/cleanup')

    }catch (error) {
      console.log(error)
      toast.error('Очистка старых приемов')
    }
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Calendar className="h-5 w-5 text-primary" />
          Недавние записи
        </CardTitle>
        <CardDescription>Мониторьте и управляйте всеми записями к врачам</CardDescription>
      </CardHeader>

      <CardContent>
        <div className="rounded-lg border">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Пациент</TableHead>
                <TableHead>Доктор</TableHead>
                <TableHead>Дата & Время</TableHead>
                <TableHead>Причина</TableHead>
                <TableHead>Статус</TableHead>
                <TableHead className="text-right">Действие</TableHead>
              </TableRow>
            </TableHeader>
            { false ?  appointments.map((appointment) => (
            <TableBody>

                <TableRow key={appointment.id}>
                  <TableCell>
                    <div>
                      <div className="font-medium">{appointment.patientName}</div>
                      <div className="text-sm text-muted-foreground">
                        {appointment.patientEmail}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="font-medium">  <div>
                    <div className="font-medium">{appointment.doctorName}</div>
                    <div className="text-sm text-muted-foreground">
                      {appointment?.doctorEmail}
                    </div>
                  </div></TableCell>
                  <TableCell>
                    <div>
                      <div className="font-medium">
                        {new Date(appointment.date).toLocaleDateString()}
                      </div>
                      <div className="text-sm text-muted-foreground">{appointment.time}</div>
                    </div>
                  </TableCell>
                  <TableCell>{appointment.reason}</TableCell>
                  <TableCell>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleToggleAppointmentStatus(appointment.id)}
                      className="h-6 px-2"
                    >
                      {getStatusBadge(appointment.status)}
                    </Button>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="text-xs text-muted-foreground">Click status to toggle</div>
                  </TableCell>
                </TableRow>
            </TableBody>
              )) : (<TableBody  >
                  {[1,2,3,4,5,6].map((i, idx) => (
                        <Skeleton className='w-full mt-4 mx-2 h-16'/>
                  ))}
              </TableBody>)}


          </Table>
        </div>
      </CardContent>
    </Card>
  );
}

export default RecentAppointments;
